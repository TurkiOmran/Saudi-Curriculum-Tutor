...
## Inputs
A single textbook PDF, plus its grade (4, 7, or 10), subject, human-readable book name, and a stable `book_id`.
...
7. **Delete-then-add into Chroma** — any prior chunks for this `book_id` are deleted from the grade-specific Chroma collection (`grade_4`, `grade_7`, `grade_10`) so stale pages can't linger.
