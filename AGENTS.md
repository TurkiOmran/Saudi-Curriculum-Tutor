...
| Find a historical agentic-layer decision     | `RESPONSE_WORKFLOW.md` (L1–L22, pre-merge)    |
...
Precedence when they disagree: `docs/WORKFLOW_SANDBOX.md` (current) > `RESPONSE_WORKFLOW.md` (historical) > `BUILD_SPEC.md`.

## Working in this repo

- **The tool-calling agent is the shipping pipeline on `main`.** The earlier `workflow-sandbox` branch was merged in `f20595a`; its single tool-calling agent + topical verifier per `docs/WORKFLOW_SANDBOX.md` replaced the two-graph LangGraph pipeline. `RESPONSE_WORKFLOW.md` (L1–L22) is preserved as the historical comparison baseline.
...
- Read `BUILD_SPEC.md`, `RESPONSE_WORKFLOW.md`, AND `docs/WORKFLOW_SANDBOX.md` before proposing any architectural change. Decisions in those docs came out of grilling sessions; revisiting one is a new grilling round, not an off-the-cuff suggestion.
